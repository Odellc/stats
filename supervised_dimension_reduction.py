...
model_dbq = QuadraticDiscriminantAnalysis(store_covariance=True)

model_dbq.fit(X_train, y_train)

y_hat_qda = model_dbq.predict(X_test)

pred_qda = confusion_matrix(y_test, y_hat_qda, labels=[0,1])

cf_matrix = sns.heatmap(pred_qda, annot=True, fmt='g', cbar=False)
cf_matrix.set(xlabel='Predicted', ylabel='Actual', title='Confusion Matrix - QDA Test')
plt.show()
tn_qda, fp_qda, fn_qda, tp_qda = pred_qda.ravel()
print(f'Precision on QDA test: {round(precision_score(tp_qda, fp_qda), 4)}')